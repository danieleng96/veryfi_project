...
select = r"select `Serial#`, `Bin#`, `Vendor`, `Tray Placement Pos`, `Index`, `Date_Time`, `Tray Name`, `TROSA Type` from `{table}` where `Tray Placement Pos` is NOT NULL or `Tray Placement Pos` != '' order by `Index` desc".format(table = table)
...
for row in mycursor:
    ...
    SN, BV, vendor, builder, tray_place, index, date, tray_name, product_name = row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7]
        ...
        if len(SN)== 8:
            ...
            #     product_name = 'TROSA_101'
                ...
                insert = "INSERT INTO labview.dbo.SnStatus (SerialNumber, ProductName, Location, Status, TStamp, BinValue, Vendor, TrayName, TrayPlace) \n VALUES (\'{serial}\', \'{product}\', \'WIP1\', \'Production\', \'{time}\', \'{binn}\', \'{vend}\', \'{TrayName}\', \'{TrayPlace}\') \n".format(
                    binn=BV, vend=vendor, serial=SN, time=date, TrayName = tray_name, TrayPlace = tray_place, product = product_name)
            try:
